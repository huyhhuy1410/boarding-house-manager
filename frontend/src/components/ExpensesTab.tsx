            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  value={expenseAmount}
                  onChange={(e) =>
                    setExpenseAmount(formatNumberString(e.target.value))
                  }
                  className="w-full rounded-lg border border-border bg-bg px-3 py-2.5 text-[13px] text-slate-100 transition-colors focus:border-indigo-500 focus:outline-none disabled:opacity-50"
                />
              </div>
              <div>
                ...
                >
                  ...
                  <optgroup label="Từng phòng cụ thể">
                    {rooms.map((r) => {
                      const bhName =
                        boardingHouses.find((h) => h.id === r.boardingHouseId)
                          ?.name || "";
                      return (
        ...
        {loading && expenses.length === 0 ? (
          <div className="flex flex-col divide-y divide-border">
            {Array.from({ length: 3 }).map((_, idx) => (
              <div
                key={idx}
                className="flex animate-pulse items-center justify-between bg-surface/50 px-4 py-3.5"
              >
                ...
                <div>
                  ...
                  <div className="mt-0.5 text-[11.5px] text-slate-400">
                    ...
                    <strong className="text-slate-300">
                      {(() => {
                        // Tra cứu thông tin phòng và dãy trọ từ props rooms & boardingHouses
                        if (exp.roomId) {
                          ...
                          const bhName =
                            boardingHouses.find(
                              (h) => h.id === roomObj?.boardingHouseId,
                            )?.name || "";
                          const roomName = roomObj?.name || "";
                          return bhName
                            ? `${bhName} / Phòng ${roomName}`
                            : `Phòng ${roomName}`;
                        }
                        if (
                          exp.description &&
                          exp.description.startsWith("Dãy trọ: ")
                        ) {
                          const match =
                            exp.description.match(/^Dãy trọ: ([^-\n]+)/);
